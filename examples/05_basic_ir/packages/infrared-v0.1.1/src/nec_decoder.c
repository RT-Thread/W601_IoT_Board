...
{
    if (rt_ringbuffer_get(ringbuff, (rt_uint8_t *) & (nec_data->data.nec), sizeof(struct nec_data_struct)) == sizeof(struct nec_data_struct))
    {
...
static rt_err_t nec_decoder_decode(rt_size_t size)
{
        ...
        if (size == 65)
        {
            for (rt_uint8_t i = 0; i < 65; i++)
            {
                ...
                if (read_raw_data[i].level == IDLE_SIGNAL)
                {
                    ...
                    if ((read_raw_data[i].us > (1690 - NEC_DEVIATION)) && (read_raw_data[i].us < (1690 + NEC_DEVIATION)))
                    {
                        LOG_D(" 1 LINE:%d", __LINE__);
                        command <<= 1;
                        command |= 1;
                    }
                    else if ((read_raw_data[i].us > (560 - NEC_DEVIATION)) && (read_raw_data[i].us < (560 + NEC_DEVIATION)))
                    {
                ...
                else if ((i == 64) && ((read_raw_data[i].us > (560 - NEC_DEVIATION)) && (read_raw_data[i].us < (560 + NEC_DEVIATION))))
                {
                    t1 = command >> 8;
                    t2 = command;
    ...
    else if (nec_state == 0x04)
    {
        ...
        if ((state_code[1].level == IDLE_SIGNAL) && ((state_code[1].us > 4000) && (state_code[1].us < 5000))) /* if guidance code 4500us */
        {
            nec_state = 0x01;
            LOG_D("guidance");
        }
        else if ((state_code[1].level == IDLE_SIGNAL) && ((state_code[1].us > 2150) && (state_code[1].us < 2350))) /* if repetition code 2250us */
        {
            nec_data.repeat++;
            nec_state = 0x00;
            rt_ringbuffer_put(ringbuff, (rt_uint8_t *)&nec_data, sizeof(struct nec_data_struct));
    ...
    else
    {
        ...
        if ((state_code[0].level == CARRIER_WAVE) && ((state_code[0].us > 8500) && (state_code[0].us < 9500))) /* if (guidance code or repetition code) */
        {
            nec_state = 0x04;
        }
        else
